...
class RequestSizingTest extends AnyFlatSpec with should.Matchers with Inspectors {
  ...
  private val expectedUrlSizeWithJustASingleOneCharCapiId = 44 + apiKey.length
  val idAtTheSizeLimit: CapiId = {
    val targetLengthForCapiIdThatIsOnTheLimit = MaxCapiUriLength - expectedUrlSizeWithJustASingleOneCharCapiId + 1
  ...
  it should "calculate the length of the correct parts of the url - the parts that the length limit applies to" in {
    practicalRequestSizer.urlLengthFor(Set(CapiId("a"))) shouldEqual expectedUrlSizeWithJustASingleOneCharCapiId
