...
class SearchJsonParserTest extends FlatSpec with Matchers with ClientTest {
  ...
  //  ?page-size=2&show-fields=all&show-tags=all
  lazy val searchResponse = JsonParser.parseSearch(loadJson("search.json"))

  // generated by:
  // http://content.guardianapis.com/search.json
  //  ?api-key=<PARTNER KEY>&ids=music/picture/2010/aug/25/georgemichael-ukcrime&show-media=all
  lazy val partnerSearchResponse = JsonParser.parseSearch(loadJson("search-partner.json"))

  // generated by:
  // http://content.guardianapis.com/search.json
  //  ?api-key=<INTERNAL KEY>&ids=music/picture/2010/aug/25/georgemichael-ukcrime&show-media=all&show-elements=all
  lazy val internalSearchResponse = JsonParser.parseSearch(loadJson("search-internal.json"))
