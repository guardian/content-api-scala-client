...
class ItemJsonParserTest extends FlatSpec with Matchers with ClientTest {
  ...
  lazy val tagItemResponse: ItemResponse = JsonParser.parseItem(loadJson("item-tag.json"))

  // generated by:
  // http://content.guardianapis.com/travel.json
  lazy val sectionItemResponse = JsonParser.parseItem(loadJson("item-section.json"))


  // generated by:
  // http://content.guardianapis.com/travel/2010/aug/27/britains-best-budget-eats-oxford.json
  //   ?show-related=true
  lazy val contentItemResponse = JsonParser.parseItem(loadJson("item-content.json"))
  ...
  lazy val contentItemExpiredResponse = JsonParser.parseItem(loadJson("item-content-expired.json"))
